...
Rules: as queue A below (strictly in order; after every item `npm test`, `npm run lint`, `npm run build` if `src/` changed, tick the box, `node scripts/roadmap.mjs --write`, update this file, one commit, push; open decisions to TODO.md with the safe default; never commit anything from `Photo/`, never the real address). **No deploys at all, not even the preview:** the owner tests once everything is finished.

- [x] 1. **Done 2026-09-26 (DECISIONS 82, CR-1122, CR-1123; OWN-01 and LEG-09 done).** `node scripts/portrait.mjs "<source>"` made `public/images/portrait.jpg` (1200 x 1500, progressive, 116 kB, no crop needed: the source is already 4:5 without a caption strip) plus JPEG 800/480 and AVIF 1200/800/480/240, each with IPTC `trainedAlgorithmicMedia` in its XMP; nothing else from `Photo/`. One component, `components/ui/PortraitImage.tsx` (a `<picture>`: AVIF first, JPEG fallback, `sizes` per frame), shows it in the landing frame and, new, beside the name in the About header (static page and About window alike). The label «KI-Porträt» / «AI portrait» / «پرتره‌ی هوش مصنوعی» sits inside the frame's bottom end corner (bottom left in fa), 10 px on a fixed dark veil (6.7:1 even over white), the same in dark and light, in the first screen at 1440 and 390 (measured); the alt text says «…, KI-generiert». JSON-LD: `Person.image` -> ImageObject `#portrait` with `digitalSourceType` on every indexed page of the main site (not the coming-soon pages). `/images/*` caches a day. **LCP, clean A/B on the same machine (the build with `available: false` against this one, phone profile, median of 3, three pairs):** landing FCP = LCP 704 / 712 / 676 ms before, 720 / 724 / 752 ms after (median +20 ms, inside the ~60 ms noise; LCP = FCP in every run, CLS 0); About 724 / 716 / 668 -> 720 / 736 / 724 ms; desktop unchanged (landing 128 ms). A first version with a JPEG `srcset` and React's image preload cost about +100 ms and was replaced. Checks: 270 tests (new `portrait.test.mjs`), lint, build, `a11y.mjs` 32/32 (de 1280, fa 380 touch), `apps.mjs` 175/175, `desktop.mjs` 63/63; screenshots of landing and About (de/fa, 1440/390, dark/light) in `Claude outputs/B1/`. Original wording: Portrait (OWN-01): `Photo/ChatGPT-Bild 25. Sept. 2026, 18_12_50.png` only -> `public/images/portrait.jpg` (1200 x 1500, 4:5, the crop of queue 11, progressive, under 250 kB, sharp), `PORTRAIT.available = true`, alt text de/en/fa (AI-generated), used everywhere PORTRAIT is read (landing, About, apps), no layout shift, LCP not worse; JSON-LD `Person.image` + ImageObject with `digitalSourceType` trainedAlgorithmicMedia; the AI disclosure as a very small label inside the frame's bottom corner (AA, first view, same in dark and light, mirrored in fa), nothing else on or around the portrait (DECISIONS 76); LEG-09, OWN-01, DECISIONS, CONTENT_REVIEW.
- [x] 2. **Done 2026-09-26 (output only; the site logo is unchanged, nothing in `src/` or `public/`).** `Claude outputs/logo-proposals-2/` (git-ignored): `BRIEF.md` (what the mark must say, where it lives, drawing rules, why round 1 failed); 18 sketched ideas (`work/sketches.png`) and a wordmark type test; ten refined proposals in four directions - monogram (Home, Three, Strata, Evolve, Tilde), symbol (Forward), wordmark-led (Prompt, Mono), emblem (Tile, Seal) - each drawn on a 64-unit grid with separate hand-drawn 32 and 16 px versions, a custom wordmark from three drawn families (geometric, squared, monospace; filled outlines, optical overshoots, ink-trapped shoulders), and one presentation sheet (mark dark/light/one colour/on the grid, logo dark/light/one colour, wordmark alone, favicon in a browser-tab mock at real size, 16/32 px rasterised, app icon 180/60 px, site header, beside `~$ amonel os`) plus 12 SVGs per proposal; `overview.png` ranks all ten with Claude's honest top 3 (**Three, Tile, Strata**) and lists what was dropped and why (tilde-a read as «ã», Node as a stick figure, Home key as a generic app icon, the plug-like convergence, gauges, the `>_` bezel). Every sheet was looked at; two rounds of fixes came from that (Three's crossbar, Tile's door in one colour, Strata's counter, Prompt/Mono as wordmark-only logos). The generator scripts are in `work/generator/`. Original wording: BR-01 logo round 2 (main design task): brief, at least four directions (monogram, symbol, wordmark-led, emblem), many sketches, the best 10-12 refined on a pixel grid; each in one colour, at 16 px, dark and light; custom-drawn wordmarks; output only in `Claude outputs/logo-proposals-2/` (SVGs, one presentation sheet per proposal, an overview sheet with an honest top 3); every sheet looked at.
- [x] 3. **Done 2026-09-26 (DECISIONS 83).** (a) **The rise in slow frames was the machine, not the code:** the item-1 export (b8462f4, built in a temporary worktree with the preview flag) and HEAD, served side by side and measured in alternation with `perf.mjs --mode open`: 1280 px, frames over 33 ms, 39 / 38 / 87 (baseline) vs 33 / 36 / 112 (HEAD); 390 px at 4x CPU 136 / 351 vs 145 / 150; both builds move together, the baseline alone from 136 to 351. (b) **Phone journey TBT:** a CPU profile cut into busy runs named the biggest block after FCP - the resolver's setup inside React's hydration commit (120-130 ms at 4x, a forced full layout of the journey). The setup now runs after the next frame, in two steps (measure and resolve, then ScrollTrigger and the observers a frame later); the block is gone from the profile. Before -> after, interleaved pairs against the previous export: `load-tasks.mjs --slow` TBT median 170 -> 123 ms (five pairs: 384/170/247/148/166 -> 117/210/115/137/123); `vitals.mjs --runs 3` phone journey TBT median 237 -> 233, mean 251 -> 211 ms (five pairs: 237/357/230/249/181 -> 292/254/233/146/130; within budget 2 of 5 vs 1 of 5); LCP 1.64 s and CLS 0.019 unchanged. The 200 ms budget is not reliably met yet; what is left (hydration 80-97 ms, the GSAP chunk 62-85 ms) is in TODO.md "Queue B 3". Look unchanged: 36 crossing frames at 390 and 1280 from both builds, 35 byte-identical, one without a pixel over a tolerance of 8 (`Claude outputs/B3/`); `journey.mjs` 10 configurations, `navigation.mjs` 4, `boundaries.mjs` 1280 and 380, `toolbar.mjs` all pass. Original wording: Performance follow-up (TODO.md "Queue 12"): why frames over 33 ms rose at every size since item 1; push the phone journey TBT (278 ms) toward 200 ms; no change to the look; before/after numbers.
- [x] 4. **Done 2026-09-26.** ROADMAP recounted (`node scripts/roadmap.mjs --write`): 112 items, 79 done, 8 in progress, 25 to do, weighted **77 %** (75 % before queue B; OWN-01 and LEG-09 done, SEO-03 90 %, BR-01 45 %, PERF-02 75 %). TODO.md opens with "Everything still open (2026-09-26, after work queue B)": nine owner points (logo choice, portrait look, assets and facts, proofreading and content review, legal, the preview Worker, Cloudflare at launch, real devices, open wording) and seven Claude Code points. Original wording: Docs: ROADMAP recount, PROJECT_STATE, TODO list of everything still open split into "Claude Code" and "owner".

**Queue B finished 2026-09-26. Nothing was deployed** (not the site, not the preview, not the coming-soon page). `out/` holds a real build of HEAD (with `legal.local.ts`), not the preview build; the private preview Worker still shows the build of queue A item 13, without the portrait and the resolver change.

## Work queue 2026-09-26 (resume with "continue the work queue")

**All 13 items done (2026-09-26); items 7-13 were resent by the owner the same day. Item 13 deployed the coming-soon page (version 856ab103) and the private preview (version 5192d155); the main site was never deployed.**
...
- [x] 9. **Done 2026-09-26 (findings only; TODO.md "LEG-05 audit refresh", ROADMAP LEG-05).** No element of any built page (`out/`, `soon/dist/`) loads from another origin, no stylesheet `url()` or script `fetch`/`import` does, the CSP is unchanged; the foreign URLs in the build are links or text (the authority, the e-mail provider, GitHub, licence and library comments). No cookie anywhere. Storage: the code uses seven keys on the main site (six `amonel.*` + `ao-scheme`) and the table lists exactly these seven in de, en and fa; the coming-soon page uses `ao-lang` and `ao-scheme`, both in its own section. Fonts: every shipped file (Inter 400 in seven subsets and 700, JetBrains Mono, Space Grotesk, Press Start 2P, VT323, Vazirmatn, the label subset) has its row and OFL file; the subset is allowed (no Reserved Font Name). `ext-*` keep their MIT `LICENSE`; Playwright never ships. **Two CHECK points for the owner:** the public preview Worker shows the dummy address «Musterstraße 1» in its Impressum (Access or delete after the review), and the longer list of product names on the crossing cards (nominative use; content review CR-1108). Original wording: LEG-05 audit refresh in TODO.md for everything since the first audit (BR-07..BR-10, ao-scheme, font subset, pruned CSS, illustrations, crossings, ext-* skills, Playwright devDependency, preview Worker): external requests, storage keys vs the Datenschutz table de/en/fa, font licences vs what ships. Findings only.
- [x] 10. **Done 2026-09-26 (output only; the site logo is unchanged, nothing in `src/` or `public/`).** Five original proposals in `Claude outputs/logo-proposals/` (git-ignored): 1 Apex (one unbroken stroke up to a round apex and along the baseline to the amber cursor), 2 Orbit (ring + stem = the single-storey "a", an amber satellite dot; the ring is also the wordmark's "o"), 3 Seven (seven pixels forming an A, the last amber = today, with a pixel wordmark), 4 Card (a punch card with the clipped corner, one amber hole = 1946), 5 Window (a window holding an "a" and the cursor block; the wordmark ends in the cursor). The wordmark is drawn from arcs, lines and squares, not set in a font, so each SVG looks the same everywhere. Per proposal: `-logo-dark/-light.svg`, `-mark-dark/-light.svg`, `-favicon.svg` (a thicker 16/32 px version on the night tile); `contact-sheet.png` (and `.html`) shows each on dark and light, at header size in a mock header, as the favicon at 16 and 32 px (real rasterisation, enlarged) and next to the `~$ amonel os` lockup, with a two-line note each (also in `NOTES.md`). Only the site palette (night, ink, the brand greens, cyan, amber; the light-mode values for light). Own shapes; checked against the brands they could recall (the terminal `>_` icon, window grids, rings) and kept clear of them. Original wording: BR-01: 5 original Amonel logo proposals (mark + wordmark) in the site palette, each on dark and light, header size, 16/32 px favicon, next to the `~$ amonel os` lockup. Own shapes, no resemblance to brands. Output only: SVGs + one contact sheet in "Claude outputs/logo-proposals/" with a two-line note each. Site logo unchanged.
- [x] 11. **Done 2026-09-26 (nothing committed from it; `git status` was clean after the run).** All 11 images in `Photo/` (9 PNGs 1122 x 1402, 2 JPEGs 928 x 1152) became temporary 1200 x 1500 progressive JPEGs in the session scratchpad (outside the repo): 4:5, centred, the caption cut - a white strip under five PNGs, a dark strip under one and white text on the jacket of another (both cut at row 1334, measured by row brightness) - 109-141 kB at quality 84 (mozjpeg, via the `sharp` that ships with Next; no new dependency). Each was injected into the landing portrait box in the browser (the placeholder replaced by exactly the `<img class="h-full w-full object-cover">` Portrait.tsx renders), de, 1440 x 900 and 390 x 844 touch, dark and light: 44 screenshots plus `contact-sheet.png` with the file names in `Claude outputs/portrait-preview/`. The frame, outline and layout are untouched (DECISIONS 76). **For the owner:** all eleven are AI-generated (ChatGPT, Codex, "Generated Image") - see TODO.md "Portrait: AI-generated images" before choosing one. Original wording: Portrait preview, nothing committed: each image in Photo/ as a temporary 1200x1500 progressive JPEG (4:5, caption strip cut, <250 kB) outside the repo, injected into the landing portrait box in the browser at screenshot time; de 1440 and 390, dark and light; contact sheet with file names in "Claude outputs/portrait-preview/". git status unchanged.
- [x] 12. **Done 2026-09-26.** Item 1 repeated with the same commands on the build of d9b755a..69f6a9a (items 7-8 are the last `src/` changes; a real build, not the preview): `vitals.mjs --runs 3` (de), `sizes.mjs`, `perf.mjs --mode open` at 390 x 844 4x CPU, 768 x 1024 and 1024 x 768 touch 2x CPU, 1280 x 800. `perf.mjs` ran twice (once `--quiet`, once in full for the worst frames); both runs are given where they differ. Before = item 1 (build of b8462f4), after = now:

  | Measure | Before (item 1) | After (item 12) |
  |---|---|---|
  | phone landing FCP = LCP / CLS / TBT | 804 ms / 0 / 89 ms | **672 ms** / 0 / 81 ms |
  | phone journey LCP / CLS / TBT | 1664 ms / 0.019 / **422 ms** | 1608 ms / 0.019 / **278 ms** (budget 200) |
  | phone desktop LCP / TBT | 2472 ms / 0 | 2508 ms / 0 (budget 2.5 s, borderline as before, noise about 60 ms) |
  | phone About LCP / CLS / TBT | 788 ms / 0.025 / 85 ms | **672 ms** / 0.025 / 88 ms |
  | desktop landing / journey / desktop / About LCP | 112 / 232 / 840 / 104 ms | 112 / 236 / 836 / 96 ms (CLS at most 0.023, TBT 0) |
  | landing JS / CSS (gzip) | 140.9 kB in 10 files / 25.9 kB | **125.6 kB** in 10 / **11.4 kB** |
  | journey JS / CSS | 236.7 kB in 17 / 25.9 kB | 235.6 kB in 19 / 26.0 kB |
  | desktop JS / CSS | 153.4 kB in 11 / 25.9 kB | 151.4 kB in 13 / 26.0 kB |
  | HTML landing de / en / fa | 16.4 / 16.4 / 17.0 kB | 16.1 / 16.0 / 16.6 kB |
  | HTML journey de / en / fa | 51.4 / 49.6 / 46.6 kB | 51.6 / 49.9 / 46.8 kB |
  | HTML desktop de | 8.1 kB | 9.1 kB (en 8.9, fa 9.5: the hidden desktop text of item 7) |
  | perf 390, 4x: fps / p95 / worst / frames > 33 ms / long tasks | 54.2 / 33.3 / 66.7 ms / 150 / 2 (57 ms) | 53.3 and 51.2 / 33.3-33.4 / 66.7 ms / 198 and 254 / 4 (73) and 6 (68) |
  | perf 768 x 1024 touch, 2x | 57.8 / worst 33.4 ms / 60 / 0 | 57.5 and 57.2 / worst 50 ms / 69 and 84 / 0 |
  | perf 1024 x 768 touch, 2x | 58.0 / worst 49.9 ms / 63 / 0 | 56.5 and 56.9 / worst 50.1 ms / 105 and 97 / 0 |
  | perf 1280 x 800 | 58.7 / worst 50.1 ms / 44 / 0 | 58.0 and 57.9 / worst 50.1 ms / 69 and 72 / 0 |

  Reading: the load work of items 2b, 3a-3c is visible (phone landing and About about 130 ms faster, landing CSS -56 %, journey TBT -34 %); everything else is within the noise of this machine. **One thing is not:** frames over 33 ms rose at every scroll profile (tablet and desktop +15 to +60 %, fps -0.6 to -1.5, long tasks still 0; the phone band 51-53 fps sits slightly below item 1's 54.2, but item 2b already measured 52.5). Item 8's ruler translate was ruled out by an A/B on the same build (the declaration removed from the built stylesheet: 51.8 fps, 217 frames over 33 ms, 5 long tasks - the same band; restored). Not traced further in this queue (TODO.md "Queue 12"). Journey TBT is still above its 200 ms budget, the phone desktop LCP on its 2.5 s edge. Original wording: Final measurement: repeat item 1 exactly; before -> after table in PROJECT_STATE.md.
- [x] 13. **Done 2026-09-26.** Coming-soon page: `npm run build:soon` (progress 75 %), `npx wrangler deploy` in `soon/` -> Worker `silent-lake-8ae2`, **version 856ab103-b76a-47cb-aceb-63721c632d19**; `live-soon.mjs` all passed, `soon.mjs --base https://ahmadreza.de/` 208/208; live `/fa/` now `lang="fa"`, `inLanguage` `fa`, ProfilePage `dateModified` 2026-09-26 (item 7). Private preview: `legal.local.ts` moved outside the repo (checksum noted), `AMONEL_PREVIEW_BUILD=1 npm run setup:preview` + `npm run build` (dummy address), `out/robots.txt` = `Disallow: /`, `out/sitemap.xml` removed, `X-Robots-Tag: noindex, nofollow` appended to `out/_headers`; 167 text files of `out/` scanned against the real address: 0 hits (12 carry the dummy); `legal.local.ts` restored before the deploy, **same checksum**; deployed with a throwaway config outside the repo -> Worker `amonel-preview`, **version 5192d155-ba58-434d-8b7b-d227eeb02dde**, https://amonel-preview.ahmadrezataherieng.workers.dev: `/`, `/amonel/`, `/desktop/`, `/impressum/`, `/robots.txt` 200 with `X-Robots-Tag: noindex, nofollow`, `/sitemap.xml` 404, the Impressum shows the dummy, item 7 visible (fa `lang`, desktop title). The main site was not deployed; `out/` holds the preview build - rebuild before any real deploy. Original wording: Redeploy coming-soon (build:soon, npx wrangler deploy in soon/, live-soon.mjs, soon.mjs --base https://ahmadreza.de/); then rebuild with AMONEL_PREVIEW_BUILD=1 and update amonel-preview as PROJECT_STATE.md describes (dummy address, noindex, legal.local.ts restored same checksum). Never deploy the main site.
