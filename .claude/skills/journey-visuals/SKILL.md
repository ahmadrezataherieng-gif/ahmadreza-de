...
- **The first mount must not restyle the page** (DECISIONS 78): `ThemeProvider` writes no variables for the default theme (`:root` holds them; `bootstrap-tokens.test.mjs` pins that), the journey's `:root` block (`FIRST_ERA_CSS`) keeps the modern font stacks and page background, sections after the first are rendered with `data-off-screen`, and the resolver's `last` values start from the inline style (or the registered initial value). Find the next blocker with `load-tasks.mjs --slow`.
