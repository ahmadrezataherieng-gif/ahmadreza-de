...
const COMPONENTS: Record<AppId, LazyExoticComponent<ComponentType<AppProps>>> = {
  ...
  // 63; the Time Machine, DECISIONS.md 64). The rest share one stand-in: the
  // scheduler and the file tree, still to come.
  ...
  'time-machine': load(() => import('@/components/apps/time-machine/TimeMachineApp'), 'TimeMachineApp'),
};
...
const SIZES: Partial<Record<AppId, AppDefinition['size']>> = {
  ...
  'time-machine': { width: 760, height: 620 },
};
