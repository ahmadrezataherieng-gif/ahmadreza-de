...
## 71. Line drawing in CSS, apps handing things to each other, a preload after the build (2026-09-24, APP-09, APP-13, APP-16, PERF-09)

- **Line drawing without DrawSVGPlugin (APP-09).** ROADMAP asked for GSAP's free DrawSVGPlugin. The journey's rules are no per-frame JS and no GSAP timeline per era (journey-visuals skill); a DrawSVG effect scrubbed by scroll is exactly that. The same effect is `.ao-draw` in `globals.css`: `pathLength="1"` on the line, `stroke-dasharray: 1`, and `stroke-dashoffset` walking from 1 to 0 over a stretch of `--era-progress` (`--draw-on`, `--draw-span`), so no plugin is bundled and nothing runs per frame. First use: the four links of the 2024 dashboard's region map draw one after another between progress 0.19 and 0.24. ScrollSmoother stays unused. Not for `vector-effect: non-scaling-stroke` lines (the sparklines): their dashes are in screen units, so `pathLength` does not normalise them.
- **Apps handing things to each other (APP-13, APP-16).** One small module, `lib/app-handoff.ts`: `requestApp(id)` fires a window event that both shells listen for (the window manager opens a window, the phone pushes its history entry, so no app needs to know which shell it is in), and a value waits for the receiving app, which takes it once it is mounted and ready (the Assistant when its index is loaded, the Traceroute app at once). The last value wins, blank ones are dropped, nothing is stored or sent anywhere. Users: the Terminal's `ask <question>` and the "trace this host" buttons of Ping and DNS.
- **A preload after the build (PERF-09).** The shell is client-only, so its chunk is named nowhere in the page. `scripts/preload-shell.mjs` (part of `npm run build`) finds the chunk by a string only the Shell renders (`data-shell-layout`) and adds a `<link rel="preload">` to the three desktop pages of `out/`. On the slow-phone profile the desktop view's LCP went from 2600 to 2464 ms (median of seven cold runs each) - about the one round trip it removes; run-to-run noise is about 0.1 s. If the chunk cannot be found the script warns and leaves the pages alone; `preload-shell.test.mjs` fails then.

## 72. How the K6 style reached the main site (2026-09-24 and 25, BR-06)

- **A scoped token layer, not a second theme.** `[data-style='k6']` in `globals.css` overrides the same `--ao-*` tokens the theme engine writes on `<html>`, so every semantic class follows and there is no parallel set of rules. Dark is the default; `prefers-color-scheme: light` gives cream and forest green, as on the coming-soon page. The engine's contract is untouched (one root, `setTheme` the only entry point).
- **Where it is set:** the landing `<main>`, the About page and the legal pages' root `<div>`, and the 404 page's `<body>` (so the page colour fills the window). The journey eras and the desktop shell never carry it. UI chrome (`Button`, `LanguageSwitcher`, the footer links) takes the K6 look through Tailwind's `in-data-[style=k6]:` variants, so the same components stay era-styled inside the journey and the desktop.
- **Fonts:** the five files of `soon/fonts/` are also in `public/fonts/`, byte for byte (guarded by `soon-style.test.mjs`), with their licences (LEG-15); `font-display: swap`, nothing from another origin. The landing's film grain and green/amber glow are off under K6 (`.ao-landing-*`), the name is plain ink.
- **Checked by:** `k6-style.test.mjs` (WCAG AA on every ground in both schemes, every engine token set, fonts licensed, nothing leaks out of the scope), axe over the landing, About, Impressum, Datenschutz and 404 in de and fa (`a11y.mjs`), the journey and landing checks (`journey.mjs` de, fa, phone).
- **Left open (TODO.md):** the desktop shell (Act 3) and its `modern` theme stay as they were; whether they should follow is the owner's call.

## 73. BR-06 K6 look reverted by owner after visual comparison; old look restored; light mode and fonts decided by owner from screenshots (2026-09-25, BR-07, BR-08)

- **What happened.** The owner compared the old and the new design side by side (`Claude outputs/compare/`) and chose the old one. The visual part of BR-06 (DECISIONS.md 72) and the K6 look of the coming-soon page (DECISIONS.md 70) are reverted; the four held K6 fixes (forest-green mark, About footer alignment, right-edge strip, name clamp) are not applied as such.
- **How: a targeted revert, not a git revert.** Reference for the main site is ad0a7cd (the parent of 69f84b3), for the coming-soon page 03300d5. Only the look goes back: the `[data-style='k6']` layer and its light mode, the `in-data-[style=k6]:` variants on Button, LanguageSwitcher and the footer, the K6 font files in `public/fonts/` and `k6-style.test.mjs`. The site's own pages are dark-only again, as before.
- **What stays, and why.** All content, the app, SEO, entity and performance work, the 301s and the tests - none of it was part of the look. Vazirmatn stays the Persian font everywhere (the owner does not want IBM Plex Sans Arabic back); on the coming-soon page it replaces the system Tahoma of 03300d5. Persian never gets letter-spacing: `.ao-site-page` marks the four page roots and resets it there, and Persian tagged inside a German or English page (the switcher's "فارسی") takes Vazirmatn instead of a system fallback. The non-colour fixes of a9bcd3c stay: the tighter phone header, the Datenschutz heading that wraps on a phone, the Impressum address aligned in RTL.
- **What the owner decides next, from screenshots (BR-08):** light mode yes or no (a proposal in the old palette on the unmerged branch `proposal/light-old-palette`), and one font per role - name and headings, Latin body text, terminal/mono - from three self-hostable OFL candidates each. Then the coming-soon page is redeployed.

## 74. Fonts per role and a light mode by choice (2026-09-25, owner)

- **Fonts.** Persian: Vazirmatn everywhere. JetBrains Mono only for code, terminal and technical text (commands, prompts, file names, the "~$ amonel os" line, the e-mail address; `.ao-tech` on the site, `--font-mono` on the coming-soon page). Name and all headings: Space Grotesk (500 and 700, self-hosted, OFL). Body text, descriptions, buttons, navigation, cards and the small labels: Inter. On the main site this applies to the landing, About, legal and 404 pages (`.ao-site-page`); the era themes of the journey and desktop keep their own faces. The coming-soon page now ships Space Grotesk, Inter and JetBrains Mono too (6 Latin woff2 files, `soon/fonts/`). Persian never gets the mono face's wide word space (Vazirmatn first in every stack).
- **Light mode.** `proposal/light-old-palette` merged and deleted. Dark is the default always; the OS setting is never followed. A sun/moon button in the header corner (landing, About, Impressum, Datenschutz, 404, coming-soon and its legal pages) sets `data-scheme="light"` on `<html>`. The choice is written to localStorage (`ao-scheme`) only after a click, applied by an inline head script before first paint, dark when storage fails; no cookie. Datenschutz (site and coming-soon scope, de/en/fa, German binding) gets a table row and one sentence: stored only in the visitor's own browser after the click, never sent to the server, deletable in the browser (§ 25 Abs. 2 Nr. 2 TDDDG).
- **Coming-soon light** is recoloured to the main site's old palette: cyan accent (#0b7285), mint and amber as soft page glows, mint bar ends and terminal line, not forest green.
- **Fixes.** The darker strip at the right edge of the site's own pages came from `scrollbar-gutter: stable` (the gutter showed the root colour, not the landing glow): `auto` on those pages. Space Grotesk's narrow word space is widened in headings on the coming-soon page.

## 75. Illustrations and learning snippets in one system (2026-09-25, owner, BR-09)

- **What.** The final design of DECISIONS 74 stays. The pages were too plain, dark and text-only, so they get illustrations, icons and small "Wussten Sie schon?" details - in one system, not page by page. The scene of the coming-soon page at e365de3 (removed in 03300d5 by mistake) comes back in that system.
- **One place.** `src/components/illustrations/`: `icons.ts` (the icon set on a 24 px grid as plain path data, with a string renderer for the static coming-soon pages), `illustrations.css` (stroke, tones, snippet box, motion; no colour values) and the React components. `globals.css` and `soon/tokens.css` define the same `--ao-ill-*` tokens (cyan, mint, amber, edge, body, screen, star, glow, snippet text colours) for dark and light. `build-soon.mjs` injects the CSS, `soon-pages.mjs` replaces `<!--@icon:name-->` with the same icons.
- **Style rules.** Inline SVG only, no images, no external request. One stroke width (1.5 px on screen, `vector-effect: non-scaling-stroke`), round caps and joins. Bodies navy (white in light mode) with an edge line; only signals (data, lamps, cursors) carry cyan, mint or amber; soft glow on signals only, none in light mode. Motion is slow CSS (flowing data, orbit, blinking cursor, twinkling stars, floating) inside `prefers-reduced-motion: no-preference`: still and complete otherwise. Decorative art is `aria-hidden`.
- **Layout rules.** At most one large illustration per page; small icons beside card and section headings; at most three snippets per page, in natural gaps (beside a section lead, under the scene, beside the closing text), never over text; on phones the scene moves below the intro. Legal pages stay sober (no illustrations). The journey eras and the desktop OS are not touched.
- **Snippets.** Real text in de/en/fa (PLACEHOLDER, CONTENT_REVIEW 36), a code line as machine text, label first, then the code, then the explanation. The label is «Wussten Sie schon?» / «می‌دانستید؟» rather than the requested «Wusstest du?» / «می‌دانستی؟»: the site addresses the visitor as «Sie» and «شما» (CLAUDE.md).

## 76. Nothing is drawn on or around the portrait (2026-09-25, owner, BR-09 fix)

- **Rule.** Nothing is ever drawn on, behind, around or overlapping the portrait area; it stays clean for the real photo. The landing galaxy (OrbitScene) is removed with its SVG; the fact and mode icons stay. The one snippet sits under the portrait at exactly its width (hidden on phones). Coming-soon chips: one line from 1000 px, a two-column grid below. About route drawing: solid cyan and amber lines, no dashes, no stars.

## 77. Smooth crossings first, then crossing animations (2026-09-25, owner, PERF-02, PERF-03, BR-10)

- **Report.** The owner filmed the journey on an Android phone in Firefox: scrolling is mostly fine, but at every era crossing the image froze for 130-390 ms (worst 1946-1956 and 1971-1981). He wants no hitch, and each crossing to become a real animation of the technologies between the two eras (BR-10, step 2 of the same task).
- **Causes, measured** on the 390 px touch profile at 4x CPU throttle (`perf.mjs --profile`, a scratch script that puts every long frame at its place in the crossing, and Chrome traces): (1) the theme hand-over at the crossing's midpoint wrote 25 custom properties on `<html>`; Blink restyles every element of the page for any custom property changed there (2,800 elements, about 250 ms at 4x, also for one that nothing reads); (2) the same moment marks the era `data-started`, which unpaused 275-390 per-glyph CSS animations of the printed text at once, and ticking them cost 50-100 ms a frame for as long as the era printed; (3) `--arrival` was inherited from the scene, so the last stretch of every crossing restyled the whole era (300-460 elements) each frame. Not causes: the bridges (13-26 elements), `will-change` on the crossing layers (measured, no effect).
- **Fixes.** (1) PERF-03, with the owner's go for the engine's contract: `scopeThemeTo()` in `apply-theme.ts`. While the journey is mounted the tokens go to two targets, the chrome wrapper and the effects layer, instead of `<html>`; `<html>` keeps the first era's tokens and only `data-theme` and `data-sound` follow (cheap); `data-cursor` (its selectors reach the whole page, 220 ms) is left alone, it only matters on the desktop. Everything the journey portals outside its eras (the held puzzle dialog, the gate cue) carries its era's `data-theme-scope`. Leaving the journey re-applies the store's theme to `<html>` as before. (2) `lib/print-controller.ts`: one requestAnimationFrame scheduler sets `data-struck` on the glyphs that are due; timing identical (`--strike-delay` plus the 40 ms the old `steps(2, jump-none)` spent unstruck), reduced motion unchanged (finished state by CSS, the scheduler does not run). (3) `--arrival` is a non-inherited property written by the resolver on the four kinds of element that read it (`.ao-crt-beam`, `.ao-crt-screen`, `.ao-mac-lights`, `.ao-mac-screen`). (4) A device with a coarse pointer scrolls natively: no Lenis (its loop and `<html>` class toggles bought nothing there); programmatic glides (`scrollToEra`) use the browser's smooth scroll, reduced motion still jumps.
- **Result** (`perf.mjs --width 390 --cpu 4 --mode open`, worst frame gap per crossing, before -> after): 1956 283 -> 67 ms, 1971 317 -> 67, 1981 283 -> 67, 1984 200 -> 50, 1995 217 -> 50, today 233 -> 50; long tasks 50 -> 2. The look is unchanged (`journey.mjs` play and watch, `boundaries.mjs` at 1280 and 380 all pass; every glyph still ends struck).
- **Not verified.** Firefox on a phone: Playwright's Firefox is not installed here, so only Chromium was measured (headless, CPU-composited, so the numbers are a floor, not a phone's frame rate). The dvh-based section heights still follow the browser toolbar; each toolbar move triggers a re-measure (ROADMAP PERF-02).
- **BR-10, the crossing cards.** Each crossing out of the first six eras shows one card per technology between the two eras, in year order, scrubbed by scroll in both directions (list and years: `content/crossings.ts`, the owner's facts of 2026-09-25; the modem says «1990er» and AI «Heute» because no year was given, CONTENT_REVIEW 37). **How it fits the existing morph:** it does not replace it. The old machine still hands over in the first quarter of the crossing (`b1`-`b7` unchanged), fades out at 0.14-0.24, the era being left recedes earlier (`--recede`), the cards take 0.24-0.74 while the background changes hands behind them (the travelling edge and the theme switch still land at 0.5), and the new machine fades back in at 0.78-0.86 as the overlay lifts. The first card of 1946-56 starts as a vacuum tube that shrinks into the transistor; the first of 1956-71 is nine transistors gathering into one chip.
- **Rules kept.** Inline SVG and CSS only (23 generic drawings of 160 x 120, no logo, no trademark, only `tt-*` classes for colour), transform and opacity only, the existing scroll progress, no library, no layout per frame. Pre-built: all cards are in the HTML (+33 kB, about 7 kB gzipped per locale for the journey), hidden with `display: none`; the resolver writes `data-shot` when the current card changes and CSS shows that card and its neighbours only, so a crossing restyles a handful of elements. Each card has its own progress `--t` (registered, computed once per card) and carries the palette of the nearer era (`data-theme-scope`). Crossings got their own length (`1.2 + 0.3` viewports per card) so a card has about a third of a screen of scrolling; the tail of each era matches the next crossing's length. Reduced motion: the bridge stays, as a still grid of the same cards at their resting moment, with a caption; a visually hidden list carries the names for assistive tech and search engines in every mode. RTL: cards arrive from the left, the ruler runs right to left, the modem's phrase and names are Persian.
- **Checked.** `journey.mjs` (play de/fa/phone, watch, reduced), `boundaries.mjs` (de, phone, fa phone), `navigation.mjs`, `desktop.mjs`, `a11y.mjs` (de 1280, fa 380 touch), `crossing-a11y.mjs` (de, fa, wide, reduced: structure clean; colour contrast of a card holds by construction, axe cannot settle it under the 3D transform, `contrast.test.mjs` covers the token pairs), `sizes.mjs`, 248 tests. One real finding fixed on the way: the still caption used the muted text token on the era's page colour (2.2:1 on 1995 teal); it uses the primary token now.

## 78. The journey's viewport unit and its load cost (2026-09-26, work queue item 2, PERF-02)

- **`--ao-vh` instead of `dvh`.** Every journey height is `calc(100 * var(--ao-vh))`. The stylesheet sets it to `1lvh` (fallback `1vh`); on touch devices a script in the journey's `<head>` (`VIEWPORT_PIN_SCRIPT`, `lib/stable-viewport.ts`) pins it to pixels on `<html>` before the body exists, and `watchViewportHeight` re-pins only on a width change or a height change over 30 %. A phone toolbar (or an emulated 844 -> 780 change) no longer resizes any stage or triggers a re-measure. The static pages and the desktop keep `dvh`. Cost: with the toolbar visible the stages are as tall as the screen without it, so the bottom of a stage sits under the toolbar until the first downward scroll; the Convergence is now lvh-tall while `/desktop/` is dvh-tall, so the pixel hand-over is exact once the toolbar is hidden (after any scroll). Neither can be proved in headless Chrome: real-phone test in PERF-01.
- **Load blocking (TBT 422-523 ms -> 120-280 ms on the phone profile), found with `scripts/verify/load-tasks.mjs`:** the effects flush after hydration ran three whole-page restyles. (1) `ThemeProvider` wrote ~25 custom properties on `<html>` on every view's first mount, although `:root` already holds the `modern` values (pinned by `bootstrap-tokens.test.mjs`): the first write of the default theme now sets only the `data-*` attributes. On the journey `:root` carries the first era's tokens (`FIRST_ERA_CSS`), so that block now keeps the modern font stacks and the modern page/gutter background itself, which is what the write used to make after hydration (frames byte-identical to before). (2) The resolver toggled `data-off-screen` on every section on its first pass: sections after the first are rendered with it. (3) It wrote each section's initial progress values: `last` now starts from what the inline style holds, or the registered initial value.
- **What was measured and left alone:** scrolling itself (worst frame 50-67 ms at 4x, 0-2 long tasks over a full journey) - the load, not the eras, was the blocker; a headless-Chrome FCP on the desktop profile is bimodal (about 250 or 500 ms) on this machine, on the old build as well.

## 79. The static pages load a pruned stylesheet (2026-09-26, work queue item 3b, PERF audit P1)

- **Why.** The site is one route, so every page got the same 125 kB (25.5 kB gzip) render-blocking stylesheet; the landing page, About, the legal pages and the 404 use about a fifth of it (the rest is the journey, the desktop shell, the apps and the puzzles).
- **What.** `npm run build` ends with `scripts/prune-static-css.mjs`. It writes `static-<hash>.css` (48 kB, 10.7 kB gzip) and points the 14 static pages - in their HTML and in the RSC payload (`index.txt`) beside each - at it; the journey and desktop pages keep the full file untouched. A rule is dropped only when a selector requires a class that appears in the journey side's source (journey, puzzles, desktop shell, apps) and nowhere in the static side (the static import closure: the landing page, About, the legal pages, the 404, the layouts, no lazy imports) or in the static HTML; classes are searched as substrings of the source text, so anything unclear keeps the rule; a hand-written class assembled at run time gets its prefix protected (`ao-legal-logo-`). Element, attribute and root selectors, `@property`, `@font-face` and `@layer` order stay. `postcss` is now an explicit devDependency (it was already installed through Tailwind).
- **Prefetch.** A soft link to the journey or the desktop on the landing page used to prefetch that route's payload on sight, and with it the full stylesheet, so every landing visit downloaded what pruning had just saved. The three links (mode cards, the two "Zum Desktop" links) now prefetch on intent - pointer over, focus, touch - through `lib/intent-prefetch.ts`. Static-to-static prefetches (the language switcher) name the pruned file.
- **Proved, not assumed.** `scripts/verify/css-equal.mjs` loads every static page from the export as built and from a pruned copy and compares the computed style of every element and its `::before`/`::after`: dark and light, 1440 and 390, de/en/fa, first visit and returning visitor - 39 pages, 37 identical on the first run and the other 2 flipped an auto margin on the untouched export as well (a load-to-load flake, documented in the script), identical on rerun. Screenshots of seven pages pixel-identical; `prune-static-css.test.mjs` checks the export.

## 80. The static pages ship no message formatter (2026-09-26, work queue item 3c, PERF audit P1)

- **Why.** Every page loaded 144 kB gzip of script, 15.5 kB of it next-intl's client provider and formatjs. Three things kept it there: the root layout rendered `NextIntlClientProvider` for every view; a chunk-splitting accident merged the formatter with `next/link`, which every page needs; and any server module that imports `next-intl` (not `next-intl/server`) makes the barrel pull the provider's client reference into the page's script list.
- **What.** (1) The provider lives in `components/i18n/IntlProvider.tsx` and is used only by `JourneyRoot` (the journey's lazy chunk) and `ShellRoot` (the desktop shell's dynamic chunk); `lib/view-messages.ts` selects each view's namespaces, the page passes them down as props. (2) `next.config.mjs` puts next-intl, use-intl and formatjs in a chunk of their own (`intl`). (3) The client islands of the static pages take their words as props: `ModeChoice`, `DesktopCta` (from `Landing`), a server-rendered `StaticLanguageSwitcher` (the current view is known on the server, so no client code at all), `LegalLinks`/`SiteFooter` (a `t` function as a parameter, no message import), and `AboutContent`, which no longer has `use client`: the About app (`AboutApp`, client) wraps it with `useTranslations` and the visitor numbers, the static About page renders it on the server with `getTranslations` (About's copy is merged into the server messages in `i18n/request.ts`, never shipped).
- **Numbers.** Script per static page, gzip: 144.5 -> 128.6 kB (landing, About, legal), 144.0 -> 128.1 kB (404); the landing page's HTML no longer carries a message payload. The journey (240.8 kB) and desktop (155.1 kB) load the same code, the formatter in their lazy chunks.
- **Proved.** `css-equal.mjs` (now also comparing text, title and every link) between the export of the commit before and this one: 39 page and setup combinations, computed style of every element and pseudo-element, text and links identical; the two that flagged an animated transform and an auto margin flipping on the untouched export passed on rerun. The full matrix (apps, desktop, bonus, navigation, journey, a11y in all setups) passes.
- **Watch.** A new client component on a static page must take strings as props; `graph`-style checks are easy to write, and `chunks` per page are visible with `MSYS_NO_PATHCONV=1 node` on a resource listing (scratch script, see PROJECT_STATE).

## 81. Persian for every Persian speaker; the desktop's text for search engines (2026-09-26, work queue item 7, owner decisions on the external SEO check)

- **Persian is `fa`, not `fa-IR`.** The Persian pages speak to every Persian speaker (Iran, Afghanistan, the diaspora in Germany and elsewhere), so `<html lang>`, hreflang, the sitemap alternates and the JSON-LD `inLanguage` say `fa` on the main site and the coming-soon pages. `Intl` formats `fa` exactly as `fa-IR` (Persian digits, the Persian calendar month), so the clock and the dates did not change. German stays `de-DE`. Open Graph keeps its own format (`de_DE`, `en_US`, `fa_IR`: the protocol wants a territory), from `ogLocale` in `lib/i18n-config.ts`.
- **`dateModified` on the ProfilePage** is the date of the last commit that touched `src/` or `public/` (`lib/last-change.ts`, shared with the sitemap's `lastmod`), never a build date; on the coming-soon pages it is the build date, because that page's progress figures and their date change with every build. No date, no field.
- **No hreflang on the noindex legal pages;** they keep their canonical and the language links in the page.
- **The desktop pages carry a short static text** (what Amonel OS is, the nine programs with one line each, the seven unlockable ones with their existing descriptions) inside `.ao-sr-only`, so the desktop looks exactly as before (`navigation.mjs`'s pixel comparison of the hand-over passes) and screen readers get the same words. It has no links, so the keyboard never lands on something invisible. The copy lives in its own namespace `desktopSeo`, read on the server only and never serialised for the client. German and English desktop titles now differ (`desktopSeo.pageName`). All of it PLACEHOLDER (CONTENT_REVIEW 38).

## 82. The portrait is the owner's AI-generated image, disclosed in the frame (2026-09-26, work queue B item 1, OWN-01, LEG-09)

- **Which image.** The owner chose `Photo/ChatGPT-Bild 25. Sept. 2026, 18_12_50.png` (one of the eleven of queue 11; all are AI-generated). `node scripts/portrait.mjs <source>` makes `public/images/portrait.jpg` (1200 x 1500, 4:5, the same centre crop as the queue 11 preview: this source is already 4:5 and has no caption strip, so nothing is cut), plus `portrait-800.jpg` and `portrait-480.jpg` (progressive mozjpeg, 116 / 53 / 23 kB) and four AVIF widths (see Loading); a light unsharp mask after the resize, no EXIF. Nothing else from `Photo/` is committed.
- **Where.** Everywhere the portrait is shown: the landing frame (as before, the placeholder replaced), and, new, a small 4:5 portrait beside the name in the About header - the static About page and the About window share `AboutContent`, so both have it. One component, `components/ui/PortraitImage.tsx`, renders the picture and its label; no other file renders the image (pinned by `portrait.test.mjs`).
- **AI disclosure (EU AI Act Art. 50(4), the owner wants it unobtrusive and not as a line under the photo).** A very small text label inside the frame's bottom end corner: «KI-Porträt» / «AI portrait» / «پرتره‌ی هوش مصنوعی», 10 px, on a dark veil with fixed values (`.ao-portrait-ai`, `#e8eef4` on black at 68 %), so it reads the same in dark and light and stays at 6.7:1 or more even over a white pixel. It sits in the corner that mirrors in Persian (bottom left), inside the frame, visible on the first screen at 1440 and 390 (measured). It is `aria-hidden`, because the alt text already says it: «Porträt von Ahmadreza Taheri, KI-generiert» and the en/fa equivalents. Nothing else is drawn on or around the portrait (DECISIONS 76 unchanged).
- **Machine-readable too.** Each JPEG carries IPTC `DigitalSourceType` = `trainedAlgorithmicMedia` in its XMP (what search engines read for their "AI-generated" note). The JSON-LD gives the Person an `image` pointing at an ImageObject `#portrait` (the 1200 x 1500 file, a caption in the page's language, `digitalSourceType` `https://cv.iptc.org/newscodes/digitalsourcetype/trainedAlgorithmicMedia`) on every indexed page of the main site; the coming-soon pages do not serve the picture and do not get it. The share image stays the ProfilePage's primary image.
- **Rights (LEG-09).** There is no photographer: the task becomes the tool's terms (OpenAI assigns the output to the user) and the owner's own consent to show his likeness, which he gave by choosing it. German copyright gives an AI output little protection of its own; that only matters if someone copies it.
- **Loading.** The landing image is the page's LCP now (bigger than the name). A first version (JPEG `srcset` only, `fetchpriority="high"`, for which React also put an image preload in the head) cost the phone profile about 100 ms of FCP and LCP: on a 1.6 Mbit/s line the 23 kB image shared the bandwidth with the render-blocking stylesheet. Now a `<picture>` offers AVIF first (240/480/800/1200 wide: 4, 11, 23, 48 kB) and falls back to the JPEGs (480/800/1200); `sizes` follows the frame (11rem, 16rem, 36vw, 27rem), so a phone at DPR 1 fetches 4 kB and a DPR 3 phone 23 kB. The landing image stays `fetchpriority="high"` and eager (inside `<picture>` React adds no preload, which would name the wrong format). The About portrait is `loading="lazy"`: it is not the LCP. `/images/*` caches for a day (`_headers`), as the icons do. The frames' boxes were already fixed by aspect ratio, so nothing shifts (CLS 0 on landing and About in the check).

## 83. The journey's resolver starts after the first frame, in two steps (2026-09-26, work queue B item 3, PERF-02)

- **The "more slow frames" of queue item 12 were not the code.** The export of the item-1 commit (b8462f4) and HEAD, served side by side and measured in alternation on the same machine (`perf.mjs --mode open`): 1280 px, frames over 33 ms, baseline 39 / 38 / 87 against HEAD 33 / 36 / 112; phone profile (390 px, 4x CPU) 136 / 351 against 145 / 150. Both builds move together with the machine's state (the unchanged baseline alone swings from 136 to 351), so item 12's before/after, measured hours apart, compared two machine states. Nothing to fix; measure performance changes only as interleaved A/B pairs.
- **Where the journey's load blocked.** A CPU profile cut into the main thread's busy runs (a scratch variant of `load-tasks.mjs`) showed the largest block after FCP was the resolver's setup inside React's hydration commit: 120-130 ms at 4x CPU, most of it one forced layout of the ~3,400-element journey, paid by the first geometry read (the resolver's `measure()`, or GSAP's `getComputedStyle` in `ScrollTrigger.create`) because the commit had just dirtied layout and style.
- **Change (`Journey.tsx`, the era-detection effect):** the setup now runs after the browser's next frame (`requestAnimationFrame` then a zero timeout), when the frame has laid the page out: `measure()` and the first `resolve()` there, and `ScrollTrigger.create`, the font and resize observers and the layout-change handler one frame later still, after the browser has restyled for the values `resolve()` wrote. The body of the effect is unchanged, only wrapped (`setUp`, `connect`), and the cleanup cancels whichever step has not run yet. Until then the page shows its server-rendered state for one or two more frames, as it did for the whole second before hydration.
- **Result:** the setup block is gone from the profile (busy runs after FCP: hydration 80-97 ms, the GSAP chunk's evaluation 62-85 ms, no third one over 60 ms). `load-tasks.mjs --slow`, five interleaved pairs: TBT 384 / 170 / 247 / 148 / 166 ms before, 117 / 210 / 115 / 137 / 123 ms after (median 170 -> 123 ms). `vitals.mjs --runs 3` (the budget tool), five interleaved pairs, phone journey TBT: 237 / 357 / 230 / 249 / 181 ms before, 292 / 254 / 233 / 146 / 130 ms after (median 237 -> 233, mean 251 -> 211; within 200 ms in 2 of 5 runs against 1 of 5) - a smaller, noisier signal than the trace, because the same build swings by over 100 ms here. LCP and CLS unchanged (about 1.64 s, 0.019). The look is unchanged (36 crossing frames at 390 and 1280 from both builds: 35 byte-identical, one differing by no pixel above a tolerance of 8), and `journey.mjs` (10 configurations), `navigation.mjs` (4), `boundaries.mjs` (1280, 380) and `toolbar.mjs` pass.
- **Left, and why:** hydration of the page (80-97 ms) and evaluating the GSAP + ScrollTrigger + Lenis chunk (62-85 ms) are the two remaining blocks. Loading GSAP later (after the first frames) or not at all on touch devices (which scroll natively and do not use Lenis) would cut the second one, but GSAP's ticker drives the resolver on desktops and ScrollTrigger feeds the progress rail everywhere; replacing that is a larger change than this item allows (TODO.md).
