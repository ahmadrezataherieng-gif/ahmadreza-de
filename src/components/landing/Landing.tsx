 ...
 * mode buttons. The journey's code - GSAP, Lenis, every era - is never loaded
 * here.
 */
export async function Landing() {
  ...
  return (
    <main className="ao-landing relative isolate min-h-dvh overflow-hidden bg-background text-ink">
      <UseTheme id="modern" />
      <div className="ao-landing-glow pointer-events-none absolute inset-0 -z-10" aria-hidden="true" />

      <header className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-5 pt-5 sm:px-8">
        <p className="font-mono text-xs tracking-[0.3em] text-muted uppercase">{t('eyebrow')}</p>
      ...
      <div className="mx-auto grid w-full max-w-6xl items-center gap-x-14 gap-y-8 px-5 pt-8 pb-28 [grid-template-areas:'identity'_'portrait'_'details'] sm:px-8 md:grid-cols-[minmax(0,5fr)_minmax(0,7fr)] md:pt-14 md:[grid-template-areas:'portrait_identity'_'portrait_details'] lg:gap-x-20">
        <div className="flex min-w-0 flex-col gap-4 [grid-area:identity] md:self-end">
          <h1 className="flex flex-col font-display leading-[0.92] tracking-tight">
            <span className="text-[clamp(3rem,8.5vw,6.25rem)] font-bold text-ink">{t('firstName')}</span>
        ...
        <div className="flex min-w-0 flex-col gap-8 [grid-area:details] md:self-start">
          <dl
            ...
            className="grid grid-cols-1 gap-x-8 gap-y-4 border-y border-edge py-5 sm:grid-cols-2"
          >
            {facts.map((fact) => (
              <div key={fact.label} className="flex min-w-0 flex-col-reverse gap-0.5">
                <dt className="font-mono text-[11px] tracking-wide text-muted uppercase">{fact.label}</dt>
                <dd className="font-body text-base font-bold break-words text-ink sm:text-lg">{fact.value}</dd>
              </div>
            ))}
          </dl>
