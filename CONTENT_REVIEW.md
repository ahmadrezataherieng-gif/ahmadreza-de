...
| CR-045 | `PORTRAIT (portrait image)` | other | all | PLACEHOLDER | Since 2026-09-26 the owner's chosen AI-generated image (DECISIONS 82), on the landing page, the About page and the About window, with its label and alt text (CR-1122, CR-1123). Confirm it at the review, or replace it with a real photo (then the label and `digitalSourceType` go) |
...
| CR-1092 | `legend`, `legendNote`, `legendCount` in `src/messages/apps/timeline/{de,en,fa}.json` (APP-10): the hidden Legende badge shown in the Timeline once a period trick was used | UI label | de / en / fa | PLACEHOLDER | de «Legende» / «Sie kannten den Trick dieser Epoche.» / «Legenden-Abzeichen: 2 von 5»; en «Legend» / «You knew the trick of this era.»; fa «افسانه» / «شما ترفند این دوران را می‌شناختید.». The Persian word for the badge is a first guess |
| CR-1093 | `help.commands.ask`, `ask.handoff`, `ask.usage` in `src/messages/apps/terminal/{de,en,fa}.json` (APP-13): the help line for the new `ask` command and its two answers | UI label | de / en / fa | PLACEHOLDER | de «eine Frage an den Assistenten stellen, z. B. ask Wo arbeitet Ahmadreza?» / «Ihre Frage geht an den Assistenten.» / usage «Fragen Sie den Assistenten direkt aus dem Terminal: ask, gefolgt von Ihrer Frage, …»; en and fa parallel |
| CR-1094 | `ping.trace`, `dns.trace` in `src/messages/apps/network/{de,en,fa}.json` (APP-16): the button that hands the host to the Traceroute app | UI label | de / en / fa | PLACEHOLDER | de «Den Weg dorthin verfolgen (Traceroute)», en «Trace the way there (traceroute)», fa «دنبال کردن مسیر تا آنجا (traceroute)» |
| CR-1095 | `site.landingTitle` in `src/messages/{de,en,fa}.json` (SEO-14): the landing page's `<title>` (and its share title), a **draft** in the style of the coming-soon titles, at most 60 characters, the name first and whole, no brand suffix | SEO / meta | de / en / fa | PLACEHOLDER | de «Ahmadreza Taheri – Fachinformatiker (Ausbildung), Trier» (55), en «Ahmadreza Taheri – IT System Integration Apprentice, Trier» (58), fa «احمدرضا طاهری – کارآموز فناوری اطلاعات، تریر» (44). Replaces the former «… für Systemintegration | Amonel» titles (de 68, en 66, fa 74 characters). Choose the wording with the owner; the other pages keep «page – name | Amonel» |
| CR-1096 | The CV app: `src/messages/apps/cv/{de,en,fa}.json` (title, intro, section names, the entries «Computer- und Handyreparatur», «Frühere Stationen», «Schulbildung», «Studium oder weitere Ausbildung», «Zertifikate» with their placeholder lines) and the structure in `src/content/cv.ts` (APP-03) | app copy | de / en / fa | PLACEHOLDER | Layout is final, every entry is a marked placeholder: dates owed (OWN-05), the repair background is the owner's own statement (2026-09-24) with period, place and tasks owed, education and certificates unknown. The PDF control keeps «Der Lebenslauf folgt in Kürze.» (`os.cv.pending`). Persian marker phrase «به‌زودی اضافه می‌شود» comes from About |
| CR-1097 | The batch planner (Scheduler app, 1956 bonus): all copy in `src/messages/apps/scheduler/{de,en,fa}.json` - intro, the three methods (name and explanation), examples, job labels, results, comparison, closing line (APP-06) | app copy | de / en / fa | PLACEHOLDER | Methods: «Der Reihe nach» (FCFS), «Kürzester zuerst» (SJF), «Reihum» (round robin). Closing line: «Genau dafür gibt es Betriebssysteme: Sie entscheiden, wer als Nächstes rechnen darf …». Job letters A-F and the numbers are machine text. Check the technical wording and the Persian terms (نوبتی, برش زمانی) with a native speaker; the German title «Stapelplaner» is in `os.apps.scheduler` |
| CR-1098 | The file tree (Filesystem app, 1971 bonus): all copy in `src/messages/apps/filesystem/{de,en,fa}.json` - intro, tree and detail labels, the path explanation, the hidden-files note, the closing line (APP-07); the file contents shown come from the Terminal's own tree and copy | app copy | de / en / fa | PLACEHOLDER | Core sentence: «Ein Dateisystem ist ein Baum, und jede Datei hat einen Pfad.» Paths, file names and commands are machine text. Also: the old stand-in texts (`os.soon`, `os.bonusBody`, the `body` of the Contact, Timeline and CV entries under `os.apps`) were removed on 2026-09-24 - no app uses them any more |
| CR-1099 | `os.sound.label`, `on`, `off`, `unavailable` in `src/messages/{de,en,fa}.json` (APP-12): the label and tooltips of the sound switch in the taskbar and on the phone home screen | UI label | de / en / fa | PLACEHOLDER | de «Ton» / «Ton ist aus - zum Einschalten klicken» / «Ton ist an - zum Ausschalten klicken» / «Dieser Browser kann keinen Ton ausgeben.»; en and fa parallel. The sounds themselves are generic synthesised tones (`src/lib/sound.ts`), no recordings |
| CR-1100 | Typo fixes of the revert QA (2026-09-25, BR-07): `site.journeyDescription` in `src/messages/de.json`; `privacy.description` and the counters paragraph in `src/messages/legal/{de,en}.json`; `intro[2]` and `now.text[0]` in `src/messages/apps/about/en.json` | typo fix | de / en | PLACEHOLDER | Wording unchanged, only: de «von Ahmadreza Taheri, angehender Fachinformatiker» → «…, angehendem Fachinformatiker» (dative apposition); a hyphen used as a dash « - » → «–» in the six places listed. Doubtful points left for the owner are listed in PROJECT_STATE.md (revert QA) |
| CR-1101 | Sun/moon theme toggle labels: `nav.schemeToLight`, `nav.schemeToDark` in `src/messages/{de,en,fa}.json` (used by the site pages, the 404 and the coming-soon pages) | UI label | de / en / fa | PLACEHOLDER | de «Zum hellen / dunklen Design wechseln»; en «Switch to light / dark theme»; fa «رفتن به پوسته روشن / تیره» |
| CR-1102 | Datenschutz: table row and one sentence for the `ao-scheme` entry, site and coming-soon scope (`src/messages/legal/{de,en,fa}.json`) | legal copy | de (binding) / en / fa | PLACEHOLDER | de: «Die Wahl zwischen hellem und dunklem Design (Eintrag „ao-scheme“, localStorage) wird erst nach Ihrem Klick auf die Sonne-Mond-Schaltfläche in Ihrem eigenen Browser gespeichert, nie an den Server übertragen und lässt sich in den Browser-Einstellungen löschen (§ 25 Abs. 2 Nr. 2 TDDDG).» Legal review with LEG items |

## 36. Illustrations and learning snippets (BR-09, 2026-09-25)

The illustration system of DECISIONS 75. Every snippet is a "Wussten Sie schon?" note: label, one code line (machine text, identical in every language), one sentence. Check every fact once more with the owner; native-speaker proofreading for en and fa.

| ID | Location | Type | Languages | Status | What the real text should cover |
|---|---|---|---|---|---|
| CR-1103 | Coming-soon pages (`soon/copy.mjs`): `status` (the top line, now «Amonel · v1.0 im Aufbau», replaces «Im Aufbau»), the chips `chip1`..`chip3` («Systemintegration», «Netzwerke · Linux», «Trier, Deutschland»; the fourth chip «DE · EN · FA» is machine text) | short text | de / en / fa | PLACEHOLDER | Top line and chips from the e365de3 page. Check whether «v1.0» should be shown at all and whether the chips repeat the role line too much |
| CR-1104 | Coming-soon pages (`soon/copy.mjs`): `tipLabel` and the three snippets `tip1` (A = 65 = 01000001), `tip2` (ping, round trip), `tip3` (192.168.0.0/16, private range, 16 network bits, 65.536 addresses) | learning snippet | de / en / fa | PLACEHOLDER | de tip1 «Für einen Computer ist der Buchstabe A die Zahl 65 – gespeichert als acht Bits, also achtmal an oder aus.»; tip2 «Der Befehl ping misst, wie lange ein kleines Datenpaket zu einem anderen Rechner und wieder zurück braucht.»; tip3 «Dieser Adressbereich ist privat und für lokale Netze reserviert. Die 16 nach dem Schrägstrich heißt: Die ersten 16 Bits benennen das Netz, die übrigen 16 reichen für 65.536 Adressen.» The ping code line (example.org, 14 ms) is an invented example |
| CR-1105 | Landing page: `learn.label` and `learn.localhost` in `src/messages/{de,en,fa}.json`, the snippet under the portrait (wide screens only); code line `127.0.0.1 → localhost` in `Landing.tsx` | learning snippet | de / en / fa | PLACEHOLDER | de «Diese Adresse führt immer zum eigenen Rechner zurück: Wer sie aufruft, spricht mit sich selbst. Ihr Name ist localhost.» |
| CR-1106 | About page: `learn.dns` and `learn.chmod` in `src/messages/{de,en,fa}.json`, two snippets after the text; code lines in `AboutPage.tsx` (the DNS example address 203.0.113.10 is a documentation address, RFC 5737) | learning snippet | de / en / fa | PLACEHOLDER | de «DNS ist das Telefonbuch des Internets: Es übersetzt einen Namen wie diesen in die IP-Adresse, unter der ein Server erreichbar ist. Die Adresse hier ist ein Beispiel.» / «Unter Linux regeln drei Ziffern, wer eine Datei lesen, schreiben und ausführen darf: 7 für den Besitzer (alles), 5 für die Gruppe und für alle anderen (lesen und ausführen).» |
| CR-1107 | 404 page: `notFound.trace` in `src/messages/{de,en,fa}.json`, one snippet per language section, and the made-up traceroute (machine text, private hops) in `not-found.tsx` | learning snippet | de / en / fa | PLACEHOLDER | de «In einem Traceroute bedeutet * * *: Von diesem Abschnitt kam in der Wartezeit keine Antwort. Das Paket kann verloren sein – oder der Router antwortet einfach nicht.» |

## 37. Crossing cards: the technologies between two eras (BR-10, 2026-09-25)

Every crossing between two eras shows one card per technology, in year order, with a short label and the year (DECISIONS 77). The names and years come from the owner's list; the wording is the placeholder. Check the facts once more with the owner and have the en and fa names proofread by a native speaker.

| ID | Location | Type | Languages | Status | What the real text should cover |
|---|---|---|---|---|---|
| CR-1108 | `crossings.techs.*.name` and `.note` in `src/messages/{de,en,fa}.json`; the years and their order in `src/content/crossings.ts` (1946-56: transistor 1947, UNIVAC I 1951, magnetic-core memory 1953; 1956-71: integrated circuit 1958, IBM System/360 1964, ARPANET 1969; 1971-81: Intel 4004 1971, 8-inch floppy 1971, Ethernet 1973, Altair 8800 1975, Apple II 1977; 1981-84: mouse 1968, Xerox Alto 1973, Apple Lisa 1983; 1984-95: CD-ROM 1985, World Wide Web 1991, Linux 1991, dial-up modem; 1995-today: web search 1998, Wi-Fi 1999, cloud computing 2006, smartphone 2007, AI) | short text | de / en / fa | PLACEHOLDER | Names as small labels only, no logos (the drawings are generic). Three notes: Intel 4004 «Mikroprozessor», Xerox Alto «erste grafische Oberfläche», mouse «Vorführung» (the 1968 demo). The dial-up modem and AI carry no year of their own: «1990er» / «Heute» stand in (`crossings.when`) - decide the real wording. de «WLAN (Wi-Fi)», «KI»; fa names such as «حافظهٔ هسته‌ای مغناطیسی», «مودم تلفنی», «وای‌فای (Wi-Fi)». Whether the order inside one year (1971 chip before floppy; 1991 web before Linux) is right |
| CR-1109 | `crossings.between` in `src/messages/{de,en,fa}.json`: the line above the still row under reduced motion, and the label of the list read by screen readers (the same technologies as plain text, once per crossing) | short text | de / en / fa | PLACEHOLDER | de «Dazwischen: die Technik zwischen zwei Epochen», en «In between: the technology between two eras», fa «در این میان: فناوری‌های میان دو دوره» |
| CR-1110 | `site.description` (`src/messages/de.json`): the landing page's meta description, **trimmed** to 159 characters (was 170; a search result shows about 155-160) - only " im Browser" removed | SEO / meta | de | PLACEHOLDER | «Ahmadreza Taheri, Fachinformatiker für Systemintegration in Ausbildung in Trier, mit Hintergrund in Computer- und Handyreparatur. Portfolio als Betriebssystem.» Decide the final wording with SEO-14 |
| CR-1111 | `site.journeyDescription` (`src/messages/de.json`): the journey's meta description, **trimmed** to 151 characters (was 189) - the closing role clause ", angehendem Fachinformatiker in Trier" removed, the title carries the role | SEO / meta | de | PLACEHOLDER | «80 Jahre Computergeschichte in sieben Epochen, von ENIAC bis zur Cloud, jede mit einem kleinen Rätsel. Ein interaktives Portfolio von Ahmadreza Taheri.» |
| CR-1112 | `site.desktopDescription` (`src/messages/de.json`): the desktop's meta description, **trimmed** to 124 characters (was 181) - the closing role clause " eines angehenden Fachinformatikers für Systemintegration" removed | SEO / meta | de | PLACEHOLDER | «Amonel OS, der Desktop von Ahmadreza Taheri im Browser: Über mich, Terminal, Helpdesk-Tickets, Traceroute und Computer-Quiz.» |
| CR-1113 | `site.description` (`src/messages/en.json`): the landing page's meta description, **trimmed** to 131 characters (was 165) - the closing sentence " Portfolio as an operating system." removed | SEO / meta | en | PLACEHOLDER | «Ahmadreza Taheri, IT specialist for system integration in training in Trier, with a background in computer and mobile phone repair.» Owner: say whether the operating-system hook belongs back in (it needs a shorter role clause to fit) |
| CR-1114 | `site.journeyDescription` (`src/messages/en.json`): the journey's meta description, **trimmed** to 133 characters (was 174) - the closing role clause ", IT systems integration trainee in Trier" removed | SEO / meta | en | PLACEHOLDER | «80 years of computing in seven eras, from ENIAC to the cloud, each with a small puzzle. An interactive portfolio by Ahmadreza Taheri.» |
| CR-1115 | `site.desktopDescription` (`src/messages/en.json`): the desktop's meta description, **trimmed** to 126 characters (was 165) - the closing role clause " from an IT systems integration trainee" removed | SEO / meta | en | PLACEHOLDER | «Amonel OS, Ahmadreza Taheri’s desktop in the browser: about him, a terminal, helpdesk tickets, traceroute and a computer quiz.» |
| CR-1116 | German dashes (queue 6, typography only, no wording change): every spaced em dash « — » in `src/messages/de.json` became the German spaced en dash « – » (`eras.eniac.visual.body`, `eras.batch.insider`, `eras.dos.visual.body`, `eras.mac.visual.body`, `eras.win95.visual.body`, `eras.cloud.visual.body`, `landing.chooseLead`), and the two desktop sound tooltips «Ton ist an - …» / «Ton ist aus - …» use « – » instead of a hyphen. Left as they are: the Windows 95 window titles «Info.txt - Editor» and «Browser - Willkommen» (the period's own form) and the legal texts | typography | de | PLACEHOLDER | Read the changed sentences once; nothing else changed |
| CR-1117 | English "email" and "system integration" (queue 6, no wording change): one form per language, site and coming-soon page alike. «e-mail» / «E-mail» became «email» / «Email» in the Contact app, the Network tools (MX and TXT lines) and the coming-soon button «Send an email»; «systems integration» became «system integration» in `site.jobTitle`, `site.aboutDescription`, `site.ogAlt`, the landing fact «System integration», the About app (role, intro, apprenticeship title), the coming-soon chip and `public/llms.txt`. The assistant's search keywords keep both spellings of e-mail on purpose | terminology | en | PLACEHOLDER | Confirm the two forms. Left as they are (legal texts, not touched): the English Impressum and privacy text still say «e-mail» |
| CR-1118 | Persian consistency (queue 6, no wording change beyond one term each): «این‌جا» became «اینجا» (the 16 to 4 majority, coming-soon intro), «میز کار» became «میزکار», the loanword «دسکتاپ» became «میزکار» in the eleven visible strings that used it (landing and 404 copy, About, Network tools, Time Machine, Terminal, Quiz, the journey's assistant line, the coming-soon page), and «یکپارچه‌سازی سامانه‌ها» became «یکپارچه‌سازی سیستم» (the 15 to 6 majority) in the role line, the job facts and the About app. The assistant's keyword «دسکتاپ» stays (people type it) | terminology | fa | PLACEHOLDER | A native speaker to confirm «میزکار» (Persian Academy term) over «دسکتاپ» for the desktop, and «سیستم» over «سامانه». Left as they are: the legal text (one «سامانه‌ها» variant and one «دسکتاپ»), «زمینه‌های اصلی من … است» (plural subject with a singular verb - acceptable, to check) |

## 38. The desktop for search engines (queue 7, 2026-09-26, DECISIONS 81)

The desktop pages had only an h1 in their static HTML. They now carry a short, visually hidden text (screen readers read it too), distinct titles in German and English and fuller descriptions. All drafts by Claude Code; native-speaker proofreading for en and fa.

| ID | Location | Type | Languages | Status | What the real text should cover |
|---|---|---|---|---|---|
| CR-1119 | `desktopSeo.heading`, `.intro`, `.baseHeading`, `.base.<app>` (nine lines, one per base app), `.bonusHeading`, `.bonusIntro` in `src/messages/{de,en,fa}.json`, rendered hidden in `src/components/os/Desktop.tsx`; the bonus apps reuse their existing `os.apps.<id>.description` | SEO text (hidden) | de / en / fa | PLACEHOLDER | What Amonel OS is (a small OS in the browser, the end of the journey, the portfolio of Ahmadreza Taheri, apprentice in Trier), windows on large screens and full screen on phones, then each program in one line. de intro «Amonel OS ist ein kleines Betriebssystem im Browser und das Ende der Reise durch 80 Jahre Computergeschichte. …» It must stay true to what the apps do; never a claim the desktop does not show |
| CR-1120 | `desktopSeo.pageName` (the first part of the desktop's `<title>`; was `os.pageName`, «Desktop» in de and en) | SEO / title | de / en / fa | PLACEHOLDER | de «Portfolio-Desktop im Browser – Ahmadreza Taheri \| Amonel» (56 characters), en «Interactive portfolio desktop – Ahmadreza Taheri \| Amonel» (57), fa unchanged «میزکار – احمدرضا طاهری \| Amonel». Decide the final words with SEO-14 |
| CR-1121 | `site.desktopDescription` in `src/messages/{de,en,fa}.json`: the desktop's meta description, **fuller** (replaces the trimmed text of CR-1112 and CR-1115) | SEO / meta | de / en / fa | PLACEHOLDER | de (157 characters) «Amonel OS, der Desktop von Ahmadreza Taheri im Browser: Über mich, Lebenslauf, Terminal, Helpdesk-Tickets, Traceroute, Computer-Quiz und freischaltbare Apps.»; en (155) «… about him, his résumé, a terminal, helpdesk tickets, traceroute, a computer quiz and unlockable apps.»; fa (160) «Amonel OS، میزکار احمدرضا طاهری، کارآموز فناوری اطلاعات، در مرورگر: …» |

## 39. The portrait and its AI label (queue B item 1, 2026-09-26, DECISIONS 82)

The owner chose an AI-generated portrait. Every place that shows it (landing, About page, About window) carries a small label inside the frame, and the alt text says it too. Wording is the owner's to confirm; the label must stay visible and must keep saying that the image is AI-made (EU AI Act Art. 50(4)).
